...
export interface RedFlagRule {
  id: string;
  category: "cardiovascular" | "neurological" | "respiratory" | "anaphylaxis" | "trauma" | "general";
  keywords: string[];
  description: string;
}
...
export function assessRisk(symptomText: string, structured?: { severity?: string; duration?: string }) {
  const lower = symptomText.toLowerCase();
  const matchedRedFlags: RedFlagRule[] = [];
...
export function buildFallbackExplanation(riskLevel: RiskLevel, symptomText: string) {
  switch (riskLevel) {
    case "EMERGENCY":
      return {
